...
void setup() {
  ...
  //Initialise the Productivity state machine with desired config.
...
static void protimer_event_dispatcher(protimer_t *const obj,event_t const *const e){
  ...
  if(status == EVENT_TRANSITION){
    ...
    ee.sig = EXIT; 
    (*source)(obj,&ee); // İnstead of two code line use this || obj->active_state = source; (*obj->active_state)(obj,e);

    //2. run the entry action for the target state
    ee.sig = ENTRY;
    (*target)(obj,&ee); // İnstead of two code line use this ||(*obj->active_state)(obj,e); obj->active_state = target;
  }
